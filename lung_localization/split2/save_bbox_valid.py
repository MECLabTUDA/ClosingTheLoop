...
from config import save_bbox_valid_config as conf
...
def main():
    ...
    for i in range(len(series_list)*4):
        for j in range(4):
            if selected_image_list_valid[i] not in bbox_dict.keys():
                total_loss += abs(pred_bbox[i,j]-mean_bbox)
                not_annotated_count = not_annotated_count + 1
                print(selected_image_list_valid[i])
            else:
