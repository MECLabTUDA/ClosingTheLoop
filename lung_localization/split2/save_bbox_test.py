...
import pydicom
import glob
from config import save_bbox_test_config as conf
# need to import class definition, otherwise loading checkpoints will fail
from valid2 import AverageMeter
...
def main():
    ...
    # prepare input
    import pickle
    with open(conf['series_list_test'], 'rb') as f:
    ...
    for i, (images, image_list, selected_image_list, series_id) in tqdm(enumerate(generator), total=total_steps):
        with torch.no_grad():
            ...
            selected_image_list_test += list(selected_image_list)
            ...
            bbox_dict_test[series_id] = [int(max(0, xmin)), int(max(0, ymin)), int(min(512, xmax)), int(min(512, ymax))]


    with open(conf['bbox_out_path'], 'wb') as f:
        pickle.dump(bbox_dict_test, f, protocol=pickle.HIGHEST_PROTOCOL)
