...
import pydicom
import re
from config import extract_test_attention_maps_config as conf
...
def write_dicom(
    ...
    if not series_file_names:
        ...
        pydicom.dcmread(series_file_names[0])
        print("pydicom could read")
    ...
    filtered_image = sitk.GetImageFromArray(content_raw)
    filtered_image.CopyInformation(image3D)
    filtered_image = sitk.ScalarToRGBColormap(filtered_image, 
                                             sitk.ScalarToRGBColormapImageFilter.Jet)
    #print("Filtered Image min/max:", tmp.min(), tmp.max(), tmp.mean())
    ...
    img_255 = sitk.Cast(sitk.IntensityWindowing(image3D, windowMinimum=-600, windowMaximum=1500, 
                                             outputMinimum=0.0, outputMaximum=255.0), sitk.sitkUInt8)
    combined_volume = sitk.Cast(alpha_blend(sitk.Compose(img_255, img_255, img_255), filtered_image, mask2=mask2), 
    ...
    series_tag_values = [(k, series_reader.GetMetaData(0,k)) for k in tags_to_copy if series_reader.HasMetaDataKey(0,k)] + \
                    ...
                    ("0020|000e", series_reader.GetMetaData(0,"0020|000e") + " Saliency2"), # Series Instance UID
                    ("0020|000d", series_reader.GetMetaData(0,"0020|000d") + " Saliency2"), # Series Instance UID
...
class PEDataset(Dataset):
    ...
    def __getitem__(self,index:int):
        ...
        Returns x as the image within the loaded Bounding Box, the data path and the bounding box itself"""
        ...
        data1 = pydicom.dcmread(self.base_path+study_id+'/'+series_id+'/'+self.image_dict[self.image_list[index]]['image_minus1']+'.dcm')

        data2 = pydicom.dcmread(self.base_path+study_id+'/'+series_id+'/'+self.image_list[index]+'.dcm')

        data3 = pydicom.dcmread(self.base_path+study_id+'/'+series_id+'/'+self.image_dict[self.image_list[index]]['image_plus1']+'.dcm')
...
def main():
    ...
    # r = re.compile('[0-3][a-f0-1][a-f0-9][A-Za-z0-9]+') #userstudy list..
    # image_list_test = list(filter(r.match, image_list_test))
    # r = re.compile('[3-6][a-f0-1][acf02359][acf567][A-Za-z0-9]+') #Bonus list
    # r = re.compile('01f6c315aabb')
    r = re.compile('1b9f7f141db9')
    ...
    for ckp in checkpoint_list:
        ...
        model = medcam.inject(model, backend='gcam', output_dir= conf['output_dir'], save_maps=False, label=lambda x: 0.5 > x, layer=current_layer, return_attention=True)
        ...
        datagen = PEDataset(image_dict=image_dict, bbox_dict=bbox_dict_test, image_list=image_list_test, target_size=image_size, base_path=base_path)
        generator = DataLoader(dataset=datagen, batch_size=1, shuffle=False, num_workers=1, pin_memory=True)
        ...
        for i, (images, study_id, input_directory, bbox) in enumerate(generator):
            ...
            if current_id != study_id[0]:
                if current_id != None:
                    ...
                    write_dicom(current_filepath, model.medcam_dict['output_dir'] + "/" + model.medcam_dict['current_layer'] + "/" + str(current_id), attention_map_final)
                    attention_map = torch.zeros(0)
                    
                current_id = study_id[0]
                current_bbox = bbox
                current_filepath = input_directory
                model.medcam_dict['current_layer']  = current_layer + "/" + str(current_id)
                if not os.path.exists(model.medcam_dict['output_dir'] + "/" + model.medcam_dict['current_layer']):
                    os.mkdir(model.medcam_dict['output_dir'] + "/" + model.medcam_dict['current_layer'])
            ...
            if attention_map.size()[0] == 0:
                attention_map = torch.from_numpy(cv2.resize(a.squeeze().detach().cpu().numpy(), ((current_bbox[3]-current_bbox[1]).item(),(current_bbox[2]-current_bbox[0]).item()))).T.unsqueeze(0)
            else:
                attention_map = torch.cat([attention_map, torch.from_numpy(cv2.resize(a.squeeze().detach().cpu().numpy(), ((current_bbox[3]-current_bbox[1]).item(),(current_bbox[2]-current_bbox[0]).item()))).T.unsqueeze(0)],dim=0)
