...
from config import seresnext50_256_1class_config as conf
...
loss_weight_dict = {'negative_exam_for_pe': 1,}
...
for ep in range(num_epoch):
    ...
    model.train()
    for j, (x, mask, y_npe, series_list) in enumerate(train_generator):
    ...
    print()
    print('epoch: {}, train_loss_npe: {}'.format(ep, losses_npe.avg), flush=True)
    ...
    losses_npe = AverageMeter()
    model.eval()
    for j, (x, mask, y_npe, series_list) in enumerate(valid_generator):
        with torch.no_grad():
            start = j*batch_size
            ...
            for n in range(len(series_list)):
                # Count indeterminate cases as PE negative
                gt_list.append(max(series_dict[series_list[n]]['negative_exam_for_pe'], series_dict[series_list[n]]['indeterminate']))
    ...
    print()
    print('epoch: {}, valid_loss_npe: {}, kaggle_loss: {}'.format(ep, losses_npe.avg, kaggle_loss), flush=True)
    print()


out_dir = conf['out_dir_results']
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
np.save(out_dir+'pred_prob_list_seresnext50_256', np.array(pred_prob_list))
np.save(out_dir+'gt_list_seresnext50_256', np.array(gt_list))
np.save(out_dir+'loss_weight_list_seresnext50_256', np.array(loss_weight_list))
np.save(out_dir+'series_len_list', np.array(series_len_list))
np.save(out_dir+'id_list', np.array(id_list))

out_dir = conf['out_dir_model']
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
torch.save(model.state_dict(), out_dir+'seresnext50_256_1class')
